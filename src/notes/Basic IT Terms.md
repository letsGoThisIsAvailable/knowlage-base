...
frontend - design
backend - logic
apis to communicate between frontend and backend
search w/ [protocal]://[domain]/[site]
cloud hosting - mix of physical and virtual hosting resources and price scales for size
...
DNS - url -> ip
tcp - protocol to solve for out of order, lost, or corrupted packets but has a small delay used when order matters like text
...
http response:
   ...
   after ‘http’ version' there is a num from 100 - 599 to show if successful then ‘ok’ if good (eg. 404)
   100-199 - informational (100-continue on)
...
IMAP - manages receiving messages
SMTP - manages sending messages ()also rarely receiving)
POP - does equivalent of imap and smtp but deletes message after, now-adays used only for message automation
FTP - used for transferring files
SFTP - ftp but secure
SSH - used for accessing remote device (encrypted)
website - many pages
web app - like website but usually more interactive
webpage - one part of site
inspect element:
   elements-clientside code (double click to edit)
   console - js errors
   sources - shows all related files
   performance - shows what browser is doing and when for optimization
   memory - shows what's using most memory
libraries-reusable/versatile (many forms) so is more customisable
